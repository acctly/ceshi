$('#search').attr('action', '/web/vodtype/a.html')
var Code207 = {
    'Browser': {
        url: document.URL,
    ...
    'Mobile': {
            ...
            'Init': function() {
                if ($(".nav-slide").length) {
                    $(".nav-slide").each(function() {
                        var $that = $(this);
                        Code207.Mobile.Nav.Set($that, $that.attr('data-align'));
                    });
                }
            },
        ...
        'Mshare': function() {
            $(".open-share").click(function() {
                Code207.Browser.useragent.weixin ? $("body").append('<div class="mobile-share share-weixin"></div>') : $("body").append('<div class="mobile-share share-other"></div>');
    ...
    'Images': {
        ...
        'Qrcode': {
            'Init': function() {
                if ($("#qrcode").length) {
                    var $that = $("#qrcode");
                    Code207.Images.Qrcode.Set($that.attr('data-link'), $that.attr('data-dark'), $that.attr('data-light'));
            ...
            'Init': function() {
                if ($(".flickity").length) {
                    $(".flickity").each(function() {
                        var $that = $(this);
                        Code207.Images.Flickity.Set($that, $that.attr('data-align'), $that.attr('data-dots'), $that.attr('data-next'), $that.attr('data-play'));
                        $(this).click(function() {
                            $(".lazyload").lazyload();
                        });
                    });
            ...
            $(".myui-short").each(function() {
                ...
                if (myui.short == 1) {
                    $.ajax({
                        ...
                        success: function(r) {
                            ...
                            if (shortId.val()) {
                                ...
                                Code207.Link.Copy.Set(codyId, linkText + url_short);
                            }
                        }
                    });
                } else {
                    if (shortId.val()) {
                        ...
                        Code207.Link.Copy.Set(codyId, linkText + shortUrl);
                    }
                }
            });
        }
    ...
    'Layer': {
        ...
        'Popbody': function(name, html, day, wide, high) {
            var pop_is = Code207.Cookie.Get(name);
            var html = $(html).html();
            if (!pop_is) {
                layer.open({
                    type: 1,
    ...
    'Other': {
        ...
        'Skin': function() {
            ...
            $('.btnskin').click(function() {
                ...
                Code207.Cookie.Set('skinColor', skin, 365);
            });
            var color = Code207.Cookie.Get('skinColor');
            if (color) {
        ...
        'Search': function() {
            ...
            $(".search_submit").click(function() {
                ...
                if (!value) {
                    ...
                    $(".search_wd").val(wd);
                    debugger
                    window.location = "vodtype/a.html?name=" + $(".search_wd").val();
                } else {
                    window.location = "vodtype/a.html?name=" + $(".search_wd").val();
                }
        ...
        'History': {
            'Init': function() {
                if ($(".vod_history").length) {
                    ...
                    Code207.Other.History.Set($that.attr('data-name'), $that.attr('data-link'), $that.attr('data-pic'), $that.attr('data-part'), $that.attr('data-limit'));
                }
            },
            'Set': function(name, link, pic, part, limit) {
                if (!link) { link = document.URL; }
                var history = Code207.Cookie.Get("history");
                var len = 0;
                var canadd = true;
                if (history) {
                    ...
                    $(history).each(function() {
                        if (name == this.name) {
                            ...
                            json += "]";
                            Code207.Cookie.Set('history', json, 365);
                            return false;
                        }
                    });
                }
        ...
        'Player': function() {
            if ($("#player-left").length) {
                ...
                if (!Code207.Browser.useragent.mobile) {
        ...
        'Xunlei': function() {
            ...
            $(".Codyurl").each(function() {
                ...
                Code207.Link.Copy.Set(this, downurl);
            });
        },
        'Language': function() {
            ...
            var language = Code207.Cookie.Get('language') || myui.language;
            if (language == 1) {
                s2t();
            } else {
                t2s();
            ...
            $('.language').click(function() {
                if (language == 0) {
                    ...
                    Code207.Cookie.Set('language', 1, 365);
                } else {
                    layer.msg("正在切换简体，请稍后...", { anim: 5, time: 2000 }, function() {
                        t2s();
                        window.location.reload();
                    });
                    Code207.Cookie.Set('language', 0, 365);
                }
            });
...
$(function() {
    if (Code207.Browser.useragent.mobile) {
        Code207.Mobile.Nav.Init();
        Code207.Mobile.Mshare();
    }
    Code207.Images.Lazyload();
    Code207.Images.Flickity.Init();
    Code207.Link.Copy.Init();
    Code207.Link.Short();
    Code207.Other.Bootstrap();
    Code207.Other.Sort();
    Code207.Other.Search();
    Code207.Other.Collapse();
    Code207.Other.Slidedown();
    Code207.Other.Scrolltop();
    Code207.Other.History.Init();
    Code207.Other.Player();
    Code207.Other.Close();
    Code207.Other.Xunlei();
